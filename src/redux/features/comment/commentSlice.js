...
export const createComment = createAsyncThunk("comment/createComment", async ({postId, userId, comment}, thunkAPI) => {
  try {
    const response = await postComment({postId, userId, comment});
   return response.data; 
} catch (error) {
   return thunkAPI.rejectWithValue(error.response.data)
}
...
export const updateComment = createAsyncThunk("comment/updateComment", async ({id,postId, userId, comment}, thunkAPI) => {
  try {

      const response = await axiosIntance.put(`/comment/update/${id}`, {postId, userId, comment});
      return response.data;
  } catch (error) {
      return thunkAPI.rejectWithValue(error.response.data)
  }
